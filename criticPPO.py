...
class myModel(tf.keras.Model):
    ...
    def call(self, link_state, first_critic, second_critic, num_edges_critic, training=False):
        ...
        for _ in range(self.hparams['T']):
            ...
            mainEdges = tf.gather(link_state, first_critic)
            # Features of the neighboring links
            neighEdges = tf.gather(link_state, second_critic)
            ...
            nodesConcat = tf.concat([mainEdges, neighEdges], axis=1)
            ...
            edges_inputs = tf.math.unsorted_segment_sum(data=outputs, segment_ids=second_critic,
                                                        num_segments=num_edges_critic)
